...
class CSRFExemptSessionAuthentication(SessionAuthentication):
    ...
    X-CSRF-Token is overwritten by ADAN portal so check cannot be done when used from 'outside'
    ADAN enforces its own check so enews attack surface is fairly limited.

    FIXME : if eNews does not need to be used from 'outside' ADAN anymore, use TokenAuthentication instead
    ADAN overwrites 'Authorization header' so TokenAuthentication is not possible from outside.

    FIXME : if this still does not work,
        - disable parts of frontend that use POST/PUT/DELETE methods
        - or replace with GET methods when possible
    '''
